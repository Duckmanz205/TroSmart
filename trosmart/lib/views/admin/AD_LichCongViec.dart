...
class _AdLichCongViecState extends State<AdLichCongViec> {
  ...
  // 🌟 BIẾN ĐẾM ĐỘNG SỐ CÔNG VIỆC TRONG THÁNG
  int _countQuaHan = 0;
  ...
  Future<void> _fetchLichHens() async {
    ...
    try {
      final response = await http.get(Uri.parse('${ApiConfig.baseUrl}/LichHen'));
      if (response.statusCode == 200) {
        ...
        if (mounted) {
          setState(() {
            ...
            _calculateMonthlyStatusCounts(); // Tính toán bộ đếm động thực tế
            _isLoading = false;
          });
        }
      } else {
  ...
  String _extractEventType(String ghiChu) {
    ...
    try {
      if (ghiChu.contains('Loại sự kiện:')) {
        ...
        if (sauSuKien.contains('Ghi chú thêm:')) {
          return sauSuKien.split('Ghi chú thêm:')[0].replaceAll('.', '').trim();
        }
        return sauSuKien.replaceAll('.', '').trim();
      }
    } catch (_) {}

    if (ghiChuLower.contains('thu tiền') || ghiChuLower.contains('thutiên') || ghiChuLower.contains('tiền')) return 'Thu tiền';
    if (ghiChuLower.contains('bảo trì') || ghiChuLower.contains('sửa chữa') || ghiChuLower.contains('baotri') || ghiChuLower.contains('hư')) return 'Bảo trì';
    if (ghiChuLower.contains('gặp mặt') || ghiChuLower.contains('xem phòng') || ghiChuLower.contains('gapmat') || ghiChuLower.contains('khách')) return 'Gặp mặt';
    ...
    setState(() {
      if (_selectedTab == 'Tất cả') {
        ...
        _filteredLichHens = _allLichHens.where((item) {
          ...
          return type.toLowerCase() == _selectedTab.toLowerCase();
        }).toList();
      }
    });
  }
  ...
  // 🌟 HÀM TÍNH TOÁN BỘ ĐẾM ĐỘNG THEO THỜI GIAN THỰC TỪ DB SQL SERVER
          ...
          child: Column(
            ...
            children: [
              ...
              _isLoading
                  ...
                  : _filteredLichHens.isEmpty
                      ? Container(
                          width: double.infinity,
                          padding: const EdgeInsets.all(32),
                          decoration: BoxDecoration(color: const Color(0xFFF3EAF8), borderRadius: BorderRadius.circular(16)),
                          child: const Center(child: Text('Không có lịch trình nào', style: TextStyle(color: Colors.purple, fontSize: 13))),
                        )
                      : ListView.builder(
                          shrinkWrap: true,
                          physics: const NeverScrollableScrollPhysics(),
                          itemCount: _filteredLichHens.length,
                          itemBuilder: (context, index) => _buildTaskCard(_filteredLichHens[index]),
                        ),
  ...
  // --- WIDGET CARD ĐỒNG BỘ MÀU SẮC THEO DANH MỤC LỌC DƯỚI APP ---
  Widget _buildTaskCard(dynamic item) {
    ...
    // 🌟 KHỞI TẠO ĐỒNG BỘ MÀU THEO DANH MỤC ĐÚNG CHUẨN DESIGN
    Color tagBgColor = const Color(0xFFEAF9F5);  // Mặc định màu Mint cho 'Gặp mặt'
    Color tagTextColor = const Color(0xFF26C6DA);
    
    String cleanTag = labelTag.trim();
    if (cleanTag == 'Thu tiền') {
      tagBgColor = const Color(0xFFE2F9F3);     // Nền Emerald
      tagTextColor = const Color(0xFF2DDCB1);   // Chữ Emerald
    } else if (cleanTag == 'Bảo trì') {
      tagBgColor = const Color(0xFFE6F0FF);     // Nền Light Blue
      tagTextColor = const Color(0xFF4A90E2);   // Chữ Light Blue
    }
    ...
    return Container(
      ...
      child: ClipRRect(
        ...
        child: InkWell(
          onTap: () {
            int idLich = item['MaLichHen'] ?? item['maLichHen'] ?? 0;
            Navigator.push(
              context,
              MaterialPageRoute(builder: (context) => AdDetailLich(maLichHen: idLich)),
            ).then((_) => _fetchLichHens());
          },
          child: Padding(
