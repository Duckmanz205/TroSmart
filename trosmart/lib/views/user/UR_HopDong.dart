...
class UrHopDong extends StatelessWidget {
  ...
  Future<void> _downloadAndOpenUserPdf(BuildContext context, UserContractController controller) async {
    ...
    try {
      final response = await http.get(
        Uri.parse('${ApiConstants.baseUrl}/HopDong/${controller.maHopDong}/export-pdf'),
      );

      if (response.statusCode == 200) {
        final directory = await getApplicationDocumentsDirectory();
        
        String phongStr = (controller.contract!['soPhong'] ?? controller.contract!['SoPhong'] ?? 'Trong').toString();
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      create: (_) => UserContractController(),
      child: Scaffold(
        ...
        body: Consumer<UserContractController>(
          builder: (context, controller, child) {
            ...
            String trangThai = contract['trangThai']?.toString().trim() ?? "Chờ khách ký";
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _buildContractProgressCard(contract, contract['ngayBatDau'], contract['ngayKetThuc']),
                          ...
                          _buildPriceCard(
                            ...
                            label: 'TIỀN THUÊ / THÁNG (PHÒNG ${contract['soPhong']})',
                          ...
                          if (daKy && contract['urlChuKySupabase'] != null && contract['urlChuKySupabase'].toString().isNotEmpty) ...[
                            ...
                            Container(
                              ...
                              child: Image.network(contract['urlChuKySupabase'], fit: BoxFit.contain),
                            )
                          ],
  ...
  Widget _buildMainTitle(Map<String, dynamic> contract, String status) {
    ...
    return Column(
      ...
      children: [
        ...
        Text('HĐ-2026-P${contract['soPhong']}', style: const TextStyle(color: Color(0xFF0F172A), fontSize: 26, fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
        Container(
  ...
  Widget _buildContractProgressCard(Map<String, dynamic> contract, String? start, String? end) {
            ...
            children: [
              Column(
                ...
                children: [
                  ...
                  Text('${contract['tenCoSo']}', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                ],
              ),
  ...
  Widget _buildBottomButtons(BuildContext context, UserContractController controller, Map<String, dynamic> contract, bool daKy) {
    return Container(
      padding: const EdgeInsets.all(24),
                ...
                MaterialPageRoute(
                  builder: (context) => UrKyHopDongOnline(
                    ...
                    maKhach: int.tryParse((
                      contract['MaKhach'] ?? contract['maKhach'] ?? contract['MAKHACH'] ?? contract['ma_khach'] ?? 1
                    ).toString()) ?? 1,
                  ),
                ),
              );
