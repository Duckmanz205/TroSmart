...
class _UrLichSuXemPhongState extends State<UrLichSuXemPhong> {
  ...
  // 🌐 LẤY DANH SÁCH LỊCH HẸN TỪ SERVER
  ...
  // 🌐 CẬP NHẬT TRẠNG THÁI HỦY (ĐÃ FIX SẠCH LỖI 400 KHÔNG XÓA ĐƯỢC)
  Future<void> _handleCancelAppointment(int maLichHen) async {
    bool confirm = await showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Ông có chắc chắn muốn hủy lịch hẹn xem phòng này không?'),
        actions: [
          TextButton(
    ...
    try {
      // 🌟 FIX DỨT ĐIỂM: Sử dụng key "trangThaiMoi" bọc trong jsonEncode khớp 100% với cấu trúc Swagger
      final response = await http.put(
        Uri.parse('http://10.0.2.2:5137/api/LichHen/$maLichHen/status'),
        headers: {
      ...
      if (response.statusCode == 200 || response.statusCode == 204) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Đã hủy lịch hẹn thành công!'), backgroundColor: Colors.green),
          );
          _fetchLichSuHen(); // Tải lại dữ liệu để nhảy tab tự động
        }
      } else {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Không thể hủy lịch: $e'), backgroundColor: Colors.red),
        );
      }
    }
  }
  ...
  List<dynamic> _getFilteredAppointments() {
    if (_activeTab == 0) {
      return _allAppointments.where((item) {
        final status = item['trangThai'].toString().trim();
        return status == "Chờ xác nhận" || status == "Đã xác nhận";
      }).toList();
    } else if (_activeTab == 1) {
      return _allAppointments.where((item) => item['trangThai'].toString().trim() == "Đã xem").toList();
    } else {
      return _allAppointments.where((item) => item['trangThai'].toString().trim() == "Đã hủy").toList();
    }
  }

  Color _getStatusColor(String status) {
    switch (status) {
      case "Chờ xác nhận": return const Color(0xFF1976D2);
      case "Đã xác nhận": return const Color(0xFF2E7D32);
      case "Đã xem": return const Color(0xFF059669);
      case "Đã hủy": return const Color(0xFFD32F2F);
  ...
  Color _getStatusBg(String status) {
    switch (status) {
      case "Chờ xác nhận": return const Color(0xFFE3F2FD);
      case "Đã xác nhận": return const Color(0xFFE8F5E9);
      case "Đã xem": return const Color(0xFFD1FAE5);
      case "Đã hủy": return const Color(0xFFFFEBEE);
  ...
  Widget build(BuildContext context) {
    ...
    final upcomingCount = _allAppointments.where((item) {
      final s = item['trangThai'].toString().trim();
                        ...
                        : filteredData.isEmpty
                            ...
                            : ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  String trangThaiStr = item['trangThai'].toString().trim();

                                  // FIX TRIỆT ĐỂ LỖI ĐỎ MÀN HÌNH: Tự ghép chuỗi không cần khởi tạo locale hệ thống phức tạp
                                  String dayOfWeek = DateFormat("EEEE").format(thoiGian).toUpperCase();
                                  String monthStr = DateFormat("MM").format(thoiGian);
                                  String dayStr = DateFormat("dd").format(thoiGian);
                                  String formatDay = "$dayOfWeek, THÁNG $monthStr $dayStr";
                                  
                                  String formatTime = DateFormat("HH:mm").format(thoiGian);
                                  String formatTimeEnd = DateFormat('HH:mm').format(thoiGian.add(const Duration(minutes: 30)));
                                  ...
                                  return Padding(
                                    ...
                                    child: _buildAppointmentCard(
                                      ...
                                      date: formatDay,
                                      title: 'Phòng ${item['soPhong'] ?? item['maPhong']} - ${item['tenCoSo'] ?? "Cơ sở hệ thống"}',
                                      time: '$formatTime - $formatTimeEnd',
                                      guide: 'Ban Quản Lý Trợ Smart',
                                      contact: item['ghiChu'] != null && item['ghiChu'].toString().isNotEmpty 
                                          ? 'Ghi chú: ${item['ghiChu']}' 
                                          : 'Liên hệ: 0905123136',
                                      hasCallButton: trangThaiStr == "Đã xác nhận",
                                      hasCancelButton: trangThaiStr == "Chờ xác nhận",
                                      isPast: _activeTab != 0,
                                      onCancelPressed: () => _handleCancelAppointment(item['maLichHen']),
                                    ),
                                  );
                                },
                              ),
    ...
    return Container(
      ...
      child: Row(
        children: const [
          ...
          SizedBox(width: 8),
          Expanded(
            child: Text('Bạn đang có lịch xem phòng sắp diễn ra, chú ý điện thoại nhé!', 
              style: TextStyle(color: AppTheme.deepPurple, fontWeight: FontWeight.bold, fontSize: 12)),
          ),
        ],
      ),
    );
  ...
  Widget _buildAppointmentCard({
    ...
    String? guide, String? contact,
    bool hasCallButton = false, bool hasCancelButton = false, bool isPast = false,
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                Text(date, style: const TextStyle(color: Color(0xFF6C757D), fontSize: 11, fontWeight: FontWeight.w500)),
                Container(
            ...
            Text(time, style: const TextStyle(color: AppTheme.deepPurple, fontSize: 15, fontWeight: FontWeight.bold)),
            ...
            Row(
              children: [
                const CircleAvatar(
                  ...
                  child: Icon(Icons.person, size: 18, color: AppTheme.deepPurple),
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(guide ?? 'Ban Quản Lý', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                ...
                if (hasCallButton)
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      child: const Text('Gọi điện', style: TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.bold)),
                    ),
                  ),
                if (hasCancelButton)
                  GestureDetector(
                    onTap: onCancelPressed,
                    child: const Text('Hủy lịch', style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold, fontSize: 12)),
                  ),
              ],
            ),
          ],
